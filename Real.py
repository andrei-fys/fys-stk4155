...
def Rigde_SK(degree, X, alpha, x, y, z):
...
def Lasso_SK(degree, X, alpha,x,y,z): 
...
def OLS_CV_SKI(degree):
    """Scikit Learn method for cross validation."""
    #z = FrankeFunction(x,y)
    ax_rows, ax_cols, z = terra()
...
def Ridge_CV_SKI(degree,  alpha):
    """Scikit Learn method for cross validation."""
    #z = FrankeFunction(x,y)
    ax_rows, ax_cols, z = terra()
    x, y = np.meshgrid(ax_cols, ax_rows)
    x_train, x_test, y_train, y_test = sklearn.model_selection.train_test_split(np.c_[x.ravel(), y.ravel()], z.ravel(),test_size=0.2)#, shuffle=True)
...
if __name__ == '__main__':
    ...
    OLS_SK(degree, X, x, y, z)


    # Terrain data CV works
    #OLS_CV_SKI(degree )
    #Ridge_CV_SKI(degree,alpha)
    #Lasso_CV_SKI(degree, alpha)
