...
def one(x,y,N):
    assert len(x)==len(y)
    degree_one = x + y
    return np.hstack([np.ones((N*N,1)), degree_one])

def two(x,y,N):
    assert len(x)==len(y)
    degree_one = x + y
    degree_two = x**2 + 2*x*y + y**2
    return np.hstack([np.ones((N*N,1)), degree_one, degree_two])
...
def three(x,y,N):
    ...
    degree_two = x**2 + 2*x*y + y**2
    degree_three = x**3 + 3*x**2*y + 3*y**2*x + y**3
    return np.hstack([np.ones((N*N,1)), degree_one, degree_two, degree_three])
...
def four(x,y,N):
    ...
    degree_three = x**3 + 3*x**2*y + 3*y**2*x + y**3
    degree_four = x**4 + 4*x**3*y + 6*x**2*y**2 + 4*y**3*x + y**4
    return  np.hstack([np.ones((N*N,1)), degree_one, degree_two, degree_three, degree_four])
...
if __name__ == '__main__':
    ...
    z = FrankeFunction(x_exact, y_exact) 
    # Plot the surface.
    surf = ax.plot_surface(x_exact, y_exact, z, cmap=cm.coolwarm,
